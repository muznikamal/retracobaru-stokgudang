    ...
    <div class="max-w-6xl mx-auto bg-white shadow-lg rounded-2xl p-6 transition-all" x-data="opnamePage()">
        ...
        <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Lakukan pengecekan dan penyesuaian stok barang sesuai kondisi di
                    lapangan</p>
            </div>
        </div>

        {{-- FORM INPUT OPNAME (hanya untuk yang punya permission create) --}}
        @can('opname.create')
            <form action="{{ route('opname.store') }}" method="POST"
                ...
                <div class="flex-1" x-ref="dropdownWrapper">
                    ...
                    <x-dropdown-search name="barang_id" label="Barang" :options="$barangs->map(
                        fn($b) => [
                            'id' => $b->id,
                            'text' => $b->nama_barang,
                            'stok' => $b->stok,
                        ],
                    )" required placeholder="Cari barang..."
                        emptyMessage="Tidak ada barang ditemukan" class="mt-1" />
                </div>
                    ...
                    @forelse ($opnames as $opname)
                        ...
                        <tr class="border-t hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-center">
                                <span
                                    class="{{ $selisih > 0 ? 'text-green-600' : ($selisih < 0 ? 'text-red-600' : 'text-gray-600') }}">
                                    {{ $selisih }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center gap-2">
                                    {{-- Hanya user dengan izin edit yang bisa edit --}}
                                    @can('opname.edit')
                                    ...
                                    {{-- Hanya user dengan izin delete yang bisa hapus --}}
                                    @can('opname.delete')
        ...
        {{-- MODAL EDIT --}}
        <div class="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50" x-show="showModal"
