    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow rounded-xl p-6" x-data="{ openAdvanced: false }">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                ...
                @if (auth()->user()->hasRole('admin') || auth()->user()->hasRole('staff'))
                    <a href="{{ route('barang-keluar.create') }}"
                        class="px-5 py-2 bg-gradient-to-r from-rose-500 to-pink-600 text-white font-medium rounded-lg shadow hover:scale-105 transition">
            ...
            <div
                ...
                {{-- Search --}}
                <form action="{{ route('barang-keluar.index') }}" method="GET" class="relative w-full lg:w-1/3">
                ...
                <button @click="openAdvanced = true"
                    class="flex md:hidden items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-lg text-sm">
            ...
            <div class="overflow-visible border border-gray-200 rounded-lg relative hidden md:block">
                <table class="min-w-full table-auto text-sm relative z-10">
                    <thead class="bg-gray-100 text-gray-700 sticky top-0 z-20">
                        <tr>
                            ...
                            <th class="p-3 text-center relative" x-data="{ open: false }">
                                <div class="inline-block">
                                    ...
                                    <div x-show="open" @click.away="open = false"
                                        class="absolute left-0 mt-1 w-52 font-normal text-gray-700 bg-white border rounded-lg shadow-xl z-50 max-h-56 overflow-y-auto">
                                        <a href="{{ route('barang-keluar.index') }}"
                                            class="block px-3 py-1 hover:bg-emerald-100 text-sm">- Semua -</a>
                                        @foreach ($namaPetugasList as $petugas)
                                            <a href="{{ route('barang-keluar.index', array_merge(request()->except('filter_petugas'), ['filter_petugas' => $petugas])) }}"
                                                class="block px-3 py-1 font-normal text-gray-700 hover:bg-emerald-100 text-sm {{ request('filter_petugas') == $petugas ? 'bg-emerald-200 ' : '' }}">
                                                {{ $petugas }}
                                            </a>
                                        @endforeach
                                    </div>
                                ...
                                <div class="inline-block">
                                    ...
                                    <div x-show="open" @click.away="open = false"
                                        ...
                                        @foreach ($barangList as $barang)
                                            <a href="{{ route('barang-keluar.index', array_merge(request()->except('filter_barang'), ['filter_barang' => $barang])) }}"
                                                class="block px-3 py-1 font-normal text-gray-700 hover:bg-emerald-100 text-sm {{ request('filter_barang') == $barang ? 'bg-emerald-200 ' : '' }}">
                                                {{ $barang }}
                                            </a>
                                        @endforeach
                                    </div>
                            ...
                            <th class="p-3 text-center">
                                <a href="{{ route('barang-keluar.index', array_merge(request()->except('sort'), ['sort' => request('sort') == 'jumlah_asc' ? 'jumlah_desc' : 'jumlah_asc'])) }}"
                                    class="inline-flex items-center justify-center gap-1 select-none cursor-pointer">
                                    JUMLAH
                            ...
                            <th class="p-3 text-center  relative" x-data="{ open: false }">
                                <div class="inline-block">
                                    <button type="button" @click="open = !open"
                                        class="flex items-center justify-center gap-1 cursor-pointer select-none  transition">
                                        TIPE
                                        <span class="material-symbols-outlined text-xs">arrow_drop_down</span>
                                    </button>

                                    <div x-show="open" x-transition.opacity.duration.150ms @click.away="open = false"
                                        class="absolute left-0 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-50">

                                        <a href="{{ route('barang-keluar.index') }}"
                                            class="block px-3 py-2 font-normal  text-sm hover:bg-emerald-50 text-gray-700">
                                            - Semua -
                                        </a>

                                        <a href="{{ route('barang-keluar.index', array_merge(request()->except('filter_tipe'), ['filter_tipe' => 'penjualan'])) }}"
                                            class="block px-3 py-2 font-normal text-gray-700 text-sm hover:bg-emerald-50 
                       {{ request('filter_tipe') == 'penjualan' ? 'bg-emerald-100  text-emerald-700' : 'text-gray-700' }}">
                                            Penjualan
                                        </a>

                                        <a href="{{ route('barang-keluar.index', array_merge(request()->except('filter_tipe'), ['filter_tipe' => 'kendala'])) }}"
                                            class="block px-3 py-2 font-normal text-gray-700 text-sm hover:bg-emerald-50 
                       {{ request('filter_tipe') == 'kendala' ? 'bg-emerald-100 font-semibold text-emerald-700' : 'text-gray-700' }}">
                                            Kendala
                                        </a>
                                    </div>
                                </div>
                            ...
                            {{-- USER --}}
                            <th class="p-3 text-center align-middle relative" x-data="{ open: false }">
                                <div class="inline-block">
                                    <button type="button" @click="open = !open"
                                    ...
                                    <div x-show="open" @click.away="open = false"
                                        ...
                                        @foreach ($userList as $user)
                                            <a href="{{ route('barang-keluar.index', array_merge(request()->except('filter_user'), ['filter_user' => $user])) }}"
                        ...
                        @forelse ($data as $i => $row)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="p-3 text-center">
                                    <span
                                        class="px-2 py-1 text-xs rounded-full {{ $row->tipe_keluar == 'penjualan' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-red-800' }}">
                                        {{ ucfirst($row->tipe_keluar) }}
                                    </span>
                                </td>
                                <td class="p-3 text-center">
                                    @if ($row->catatan)
                   ...
                   transition duration-150">
                                            ...
                                            @if (auth()->user()->hasRole('admin'))
                                                <form action="{{ route('barang-keluar.destroy', $row->id) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="p-3 text-center text-gray-500">Belum ada data</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                @forelse ($data as $i => $row)
                    <div x-data="{ open: false }" class="p-4 hover:bg-gray-50 transition">
                        ...
                        <div class="flex justify-between items-center cursor-pointer" @click="open = !open">
                            ...
                            <div>
                                <h3
                                    class="text-xs px-2 py-1 rounded-full {{ $row->tipe_keluar == 'penjualan' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                                    {{ ucfirst($row->tipe_keluar) }}
                                </h3>
                                <p
            ...
            <div id="catatanModal"
                ...
                <div class="bg-white p-5 rounded-lg shadow-lg w-96">
                    <h3 class="text-lg font-semibold mb-3">Catatan Barang Keluar</h3>
            ...
            <div x-show="openAdvanced" x-transition.opacity.duration.200ms
                ...
                <div @click.away="openAdvanced = false"
                    class="relative bg-white w-11/12 max-w-md rounded-2xl p-5 shadow-lg">
                    {{-- Tombol Close --}}
                    ...
                    {{-- Filter form --}}
                    <form action="{{ route('barang-keluar.index') }}" method="GET" class="space-y-4 text-sm">
                        ...
                        {{-- Nama PO --}}
                        <x-select-dropdown name="filter_petugas" label="Nama PO/Cost" :options="$namaPetugasList->map(fn($p) => ['value' => $p, 'label' => $p])" :selected="request('filter_petugas')"
                            placeholder="Semua" />

                        {{-- Barang --}}
                        <x-select-dropdown name="filter_barang" label="Barang" :options="$barangList->map(fn($b) => ['value' => $b, 'label' => $b])" :selected="request('filter_barang')"
                            placeholder="Semua" />

                        {{-- Tipe --}}
                        <x-select-dropdown name="filter_tipe" label="Tipe" :options="[
                            ['value' => '', 'label' => 'Semua'],
                            ['value' => 'penjualan', 'label' => 'Penjualan'],
                            ['value' => 'kendala', 'label' => 'Kendala'],
                        ]" :selected="request('filter_tipe')" />

                        {{-- User Input --}}
                        <x-select-dropdown name="filter_user" label="User Input" :options="$userList->map(fn($u) => ['value' => $u, 'label' => $u])" :selected="request('filter_user')"
                            placeholder="Semua" />
