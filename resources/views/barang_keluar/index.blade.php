        ...
        <div class="bg-white shadow rounded-xl p-6" x-data="{ openAdvanced: false }">
                        ...
                        @forelse ($data as $i => $row)
                            @php
                                ...
                                $canEdit =
                                    ...
                                    ($isStaff &&
                                        $row->user_id == auth()->id() &&
                                        now()->diffInHours($row->created_at->setTimezone(config('app.timezone'))) <=
                                            24);
                            @endphp
            ...
            <div x-show="openAdvanced" x-transition.opacity.duration.200ms x-init="$watch('openAdvanced', val => document.body.style.overflow = val ? 'hidden' : 'auto')"
                ...
                <div @click.away="openAdvanced = false"
                    class="relative bg-white w-11/12 max-w-md rounded-2xl p-5 shadow-lg overflow-y-auto max-h-[90vh]">
                    <!-- Tambahkan ini -->
                    {{-- Tombol Close --}}
                    ...
                    <form action="{{ route('barang-keluar.index') }}" method="GET" class="space-y-4 text-sm">
                        ...
                        {{-- Nama PO --}}
                        <x-select-dropdown-search name="filter_petugas" label="Nama PO/Cost" :options="$namaPetugasList->map(fn($p) => ['value' => $p, 'label' => $p])"
                        ...
                        {{-- User Input --}}
                        <x-select-dropdown-search name="filter_user" label="User Input" :options="$userList->map(fn($u) => ['value' => $u, 'label' => $u])"
